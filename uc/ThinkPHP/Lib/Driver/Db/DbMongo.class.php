...
class DbMongo extends Db{
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
			...
			// -------- start ---------
			// 以下处理方式兼容mysql模式下的查询方式
			// 例:$map['system_time'][] = array('gte', strtotime($time_start . ' 00:00:00'));
			//    $map['system_time'][] = array('lte', strtotime($time_end . ' 23:59:59'));
			// Modify by 张成国 Date:2012/11/23
			else {
				$count = count($val);
				$where = array();
				for($i=0;$i<$count;$i++) {
                    $data = is_array($val[$i])?$val[$i][1]:$val[$i];
					$con  =  strtolower($val[$i][0]);
					if(in_array($con,array('neq','ne','gt','egt','gte','lt','lte','elt'))) { // 比较运算
						$k = '$'.$this->comparison[$con];
						$where[$k]  =  $data;
					}
