...
abstract class Action {
   ...
   /**
    ...
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false) {
		...
		// ----- start -----
		// 如果时间设置为0秒,则直接跳转
		// Modify by 张成国 Date:2012/11/20
		if ($this->get('waitSecond') === 0) {
			$jumpUrl = $this->get('jumpUrl');
			exit("<script type='text/javascript'>
				if ( self.parent.main == undefined ) {
					location.href = 'javascript:self.parent.location.reload();';
				} else {
					location.href = 'javascript:self.parent.main.location.reload();';
				}
			</script>");
		}
		// ----- end -----
