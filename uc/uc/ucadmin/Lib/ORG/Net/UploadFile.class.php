...
class UploadFile {//类定义开始
    ...
    public function upload($savePath ='') {
		...
		makeDir($savePath); // 自动创建目录 Modify by 张成国 Date:2012/11/24
        // 检查上传目录
        if(!is_dir($savePath)) {
            // 检查目录是否编码后的
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
				...
				// 直接返回图片的相对路径 Modify by 张成国 Date:2012/11/24
				$file['file_path'] = str_replace(C('IMG_ROOT'), '', $file['savepath']) . $file['savename'];
                //上传成功后保存文件信息，供其他地方调用
